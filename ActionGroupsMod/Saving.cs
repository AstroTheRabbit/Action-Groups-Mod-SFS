...
using SFS.World;
using UnityEngine;
using static CustomSaveData.Entrypoint;
...
namespace ActionGroupsMod
{
    ...
    {
        private const string DataId = "actionGroups";

        private static List<ActionGroupSave> CreateSaves(List<ActionGroup> actionGroups, List<Part> parts)
        {
            return actionGroups.Select(ag => new ActionGroupSave(ag, parts)).ToList();
        }

        private static List<ActionGroup> LoadSaves(List<ActionGroupSave> saves, List<Part> parts)
        {
        ...
        {
            BlueprintHelper.OnSave += Blueprint_OnSave;
            BlueprintHelper.OnLoad += Blueprint_OnLoad;
            BlueprintHelper.OnLaunch += Blueprint_OnLaunch;
            RocketSaveHelper.OnSave += RocketSave_OnSave;
            RocketSaveHelper.OnLoad += RocketSave_OnLoad;
        }

        private static void Blueprint_OnSave(CustomBlueprint blueprint)
        {
        ...
        private static void Blueprint_OnLoad(CustomBlueprint blueprint)
        {
        ...
        private static void Blueprint_OnLaunch(CustomBlueprint blueprint, Rocket[] rockets, Part[] parts)
        {
            if (!blueprint.GetCustomData(DataId, out List<ActionGroupSave> saves))
            {
        ...
        private static void RocketSave_OnSave(CustomRocketSave rocketSave, Rocket rocket)
        {
        ...
        private static void RocketSave_OnLoad(CustomRocketSave rocketSave, Rocket rocket)
        {
